...
// This exercise writes an init method to return the entire list without the last element

// I did not implement this, it is from:
  ...
  */
  ...
  def init2[A](l: List[A]): List[A] = {
    ...
    def go(cur: List[A]): List[A] = cur match {
      case Nil => sys.error("init of empty list")
      // : _* tells the compiler to treat a single argument of a sequence type as a variable argument sequence
